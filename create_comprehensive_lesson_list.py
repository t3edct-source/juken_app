...
# 各レッスンの詳細な概要を取得
def get_comprehensive_summary(item):
    """レッスンの包括的な概要を取得"""
    path = item.get("path", "")
    id_str = item.get("id", "")
    title = item.get("title", "")
        ...
        try:
            with open(js_path, "r", encoding="utf-8") as f:
                ...
                if text_matches:
                    # 最初の15問から主要な概念を抽出
                    concepts = set()
                    for text in text_matches[:15]:
                        # 重要なキーワードを抽出
                        if "何" in text or "なぜ" in text or "どの" in text or "どう" in text:
                            # 質問の内容から学習ポイントを推測
                            if "季節" in text:
                                concepts.add("季節と生物の関係")
                            if "植物" in text:
                                concepts.add("植物の構造・成長")
                            if "花" in text:
                                concepts.add("花の構造と受粉")
                            if "電気" in text or "電流" in text:
                                concepts.add("電気の基礎")
                            if "回路" in text:
                                concepts.add("回路のしくみ")
                            if "乾電池" in text or "豆電球" in text:
                                concepts.add("乾電池と豆電球")
                            if "水" in text and "水溶液" not in text:
                                concepts.add("水の性質・状態変化")
                            if "空気" in text:
                                concepts.add("空気の性質")
                            if "燃焼" in text:
                                concepts.add("燃焼のしくみ")
                            ...
                            if "音" in text:
                                concepts.add("音の性質")
                            if "溶液" in text or "溶解度" in text:
                                concepts.add("水溶液と溶解度")
                            if "酸" in text or "アルカリ" in text:
                                concepts.add("酸性・アルカリ性")
                    
                    summary["learning_content"] = sorted(list(concepts))
    ...
    for field in ["生物", "物理", "化学", "地学", "総合", "その他"]:
        if field in lessons_by_grade[grade]:
            ...
            for item in lessons_by_grade[grade][field]:
                summary = get_comprehensive_summary(item)
                ...
                if summary['learning_content']:
                    output_lines.append(f"  学習内容: {', '.join(summary['learning_content'])}")
...
# ファイルに出力
with open("lesson_summary_comprehensive.txt", "w", encoding="utf-8") as f:
    f.write("\n".join(output_lines))
