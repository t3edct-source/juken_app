...
def extract_questions(content):
    ...
    for i, qnum_match in enumerate(qnum_matches):
        ...
        obj_start = array_content.rfind('  {', 0, start_pos)
        if obj_start == -1:
            obj_start = array_content.rfind('{', 0, start_pos)
        if obj_start == -1:
            obj_start = 0
        ...
        if i + 1 < len(qnum_matches):
            next_obj_start = array_content.rfind('  {', 0, qnum_matches[i + 1].start())
            if next_obj_start == -1:
                next_obj_start = array_content.rfind('{', 0, qnum_matches[i + 1].start())
            if next_obj_start == -1:
                next_obj_start = qnum_matches[i + 1].start()
            end_pos = next_obj_start
        else:
            # 最後の問題の場合、配列の終了まで
            end_pos = len(array_content)
        
        # 問題オブジェクトを抽出（インデントを含む）
        question_block = array_content[obj_start:end_pos].rstrip()
        # 末尾のカンマを除去
...
def main():
    ...
    print("=" * 80)
    print("太陽と影（基礎）の単元 - 問題の再配置（最終修正版）")
    print("=" * 80)
    print()
