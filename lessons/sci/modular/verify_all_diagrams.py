            ...
            if has_diagram:
                ...
                if div_match:
                    ...
                    if len(non_empty_lines) >= 3:
                        print(f"  Q{qnum}: 図解あり - 【{title}】（{len(non_empty_lines)}行）")
                    else:
                        print(f"  Q{qnum}: 図解あり（内容が少ない） - 【{title}】（{len(non_empty_lines)}行）")
                else:
...
def main():
    ...
    check_all_diagrams(target_file)
