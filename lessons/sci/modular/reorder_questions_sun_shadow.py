    ...
    while i < len(array_content):
        if array_content[i] == '{':
            if brace_count == 0:
                start_pos = i
                # qnumを探す
                qnum_match = re.search(r'"qnum":\s*(\d+)', array_content[i:i+200])
                if qnum_match:
                    current_qnum = int(qnum_match.group(1))
            brace_count += 1
        elif array_content[i] == '}':
            brace_count -= 1
            if brace_count == 0 and start_pos is not None and current_qnum is not None:
                # 問題オブジェクトを抽出
                end_pos = i + 1
                question_block = array_content[start_pos:end_pos].strip()
                # 末尾のカンマを除去
                question_block = question_block.rstrip(',').strip()
                questions[current_qnum] = question_block
                start_pos = None
                current_qnum = None
        i += 1
