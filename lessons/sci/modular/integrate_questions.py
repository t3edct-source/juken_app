        ...
        "電流計の針": [
            {
                ...
                "choices": ["大きくなる", "小さくなる", "変わらない", "針の動きと電流は関係ない"],
                "answer": 0,
                "explanation": "電流計の針は、電流が大きいほど右に大きく振れる"
            },
            {
                "text": "電流計の針が0の位置にあるとき、電流の大きさはどうなりますか？",
                "choices": ["電流が流れていない", "電流が流れているが測定できない", "電流の向きが逆", "電流計が故障している"],
                "answer": 0,
                "explanation": "電流計の針が0の位置にあるときは、電流が流れていないことを示す"
            },
            {
                "text": "電流計の針の読み方として正しいのはどれですか？",
                "choices": ["針の先端の位置を読む", "針の中央の位置を読む", "針の根元の位置を読む", "針の動きを見るだけでよい"],
                "answer": 0,
        ...
        "変温動物": [
            {
                ...
                "choices": ["気温によって体温が変わる動物", "体温がほぼ一定の動物", "体温が常に高い動物", "体温がない動物"],
                "answer": 0,
                "explanation": "変温動物は、気温によって体温が変わる動物。魚類、両生類、は虫類などが該当する"
            },
            {
                "text": "変温動物の例として正しいのはどれですか？",
                "choices": ["カエル、トカゲ、メダカ", "イヌ、ネコ、ウマ", "ニワトリ、ペンギン", "クジラ、イルカ"],
                "answer": 0,
                "explanation": "変温動物には、カエル（両生類）、トカゲ（は虫類）、メダカ（魚類）などが含まれる"
            },
            {
                "text": "恒温動物とはどのような動物ですか？",
                "choices": ["体温がほぼ一定に保たれる動物", "気温によって体温が変わる動物", "体温が常に低い動物", "体温がない動物"],
                "answer": 0,
    ...
    "地学": {
        "月の模様が変わらない理由": [
            ...
            },
            {
                ...
                "choices": ["ほぼ等しい（約27.3日）", "自転周期の方が長い", "公転周期の方が長い", "関係がない"],
                "answer": 0,
        ...
        "翌日の月の位置": [
            ...
            },
            {
                ...
                "choices": ["約48分ずつおそくなる", "約48分ずつ早くなる", "変わらない", "約1時間ずつおそくなる"],
                "answer": 0,
        ...
        "月の満ち欠け周期": [
            {
                ...
                "choices": ["約29.5日", "約27.3日", "約30日", "約28日"],
                "answer": 0,
...
def get_max_qnum(content):
    """既存の問題の最大qnumを取得"""
    qnums = re.findall(r'"qnum":\s*(\d+)', content)
    return max([int(n) for n in qnums]) if qnums else 0

def format_question_js(q, qnum, is_wakaru=False):
    """問題オブジェクトをJavaScript形式の文字列に変換"""
    indent = "  "
    lines = [f"{indent}{{"]
    lines.append(f'{indent}  "qnum": {qnum},')
    lines.append(f'{indent}  "text": {json.dumps(q["text"], ensure_ascii=False)},')
    lines.append(f'{indent}  "choices": [')
    for choice in q["choices"]:
        lines.append(f'{indent}    {json.dumps(choice, ensure_ascii=False)},')
    lines[-1] = lines[-1].rstrip(',')  # 最後のカンマを削除
    lines.append(f'{indent}  ],')
    lines.append(f'{indent}  "answer": {q["answer"]},')
    ...
    lines.append(f'{indent}  "difficulty": 1,')
    lines.append(f'{indent}  "asof": "2025-01-27"')
    lines.append(f'{indent}}}')
    return '\n'.join(lines)

def add_questions_to_file(filepath, new_questions, is_wakaru=False):
    """レッスンファイルに問題を追加"""
    ...
    # 既存の最大qnumを取得
    max_qnum = get_max_qnum(content)
    
    # 配列の終了位置を探す
    # ]; の前の ] を見つける
    last_bracket_pos = content.rfind('];')
    if last_bracket_pos == -1:
        print(f"[ERROR] 配列の終了が見つかりません: {filepath}")
        return False
    ...
    for q in new_questions:
        question_js = format_question_js(q, new_qnum, is_wakaru)
        questions_js.append(question_js)
        new_qnum += 1
    
    # 挿入位置を決定
    # 最後の問題の後にカンマを追加して挿入
    insert_pos = last_bracket_pos
    
    # 既存の問題があるかチェック
    # 最後の } の位置を探す
    last_brace_pos = content.rfind('  }', 0, insert_pos)
    if last_brace_pos != -1:
        # 既存の問題がある場合、カンマを追加
        questions_text = ',\n' + ',\n'.join(questions_js) + '\n'
    else:
    ...
    # 挿入
    new_content = content[:insert_pos] + questions_text + content[insert_pos:]
    
    # ファイルに書き込み
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(new_content)
    
    print(f"[OK] {len(new_questions)}問を追加しました: {os.path.basename(filepath)}")
    return True
...
def main():
    ...
    for field, items in QUESTION_TO_LESSON.items():
        for item_name, lesson_map in items.items():
            ...
            if not questions:
                print(f"[WARN] 問題データが見つかりません: {field} - {item_name}")
                continue
            ...
            if "oboeru" in lesson_map:
                ...
                if oboeru_file.exists():
                    if add_questions_to_file(str(oboeru_file), questions, is_wakaru=False):
                        total_added += len(questions)
                else:
                    print(f"[ERROR] ファイルが見つかりません: {oboeru_file}")
            
            # wakaru版
            if "wakaru" in lesson_map:
                wakaru_file = base_dir / "wakaru" / lesson_map["wakaru"]
                if wakaru_file.exists():
                    if add_questions_to_file(str(wakaru_file), questions, is_wakaru=True):
                        total_added += len(questions)
                else:
                    print(f"[ERROR] ファイルが見つかりません: {wakaru_file}")
    
    print(f"\n完了: 合計 {total_added} 問を追加しました。")
