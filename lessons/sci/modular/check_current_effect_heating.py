...
def check_learning_order(questions):
    ...
    def categorize_question(text):
        ...
        # 基礎概念
        if '発熱' in text and ('何という' in text or '何と' in text or '作用' in text):
            return (0, '基礎概念_発熱作用')
        if '電流' in text and '熱' in text and ('何という' in text or '何と' in text):
            return (0, '基礎概念_電流と熱')
        
        # 概念（発熱の仕組み）
        if '発熱' in text and ('なぜ' in text or 'どうして' in text or '仕組み' in text):
            return (1, '概念_発熱の仕組み')
        if '電流' in text and '熱' in text and ('なぜ' in text or 'どうして' in text):
            return (1, '概念_電流と熱の関係')
        
        # 観察・実験
        if '実験' in text or '観察' in text or '調べる' in text:
            return (2, '観察方法_実験・観察')
        if 'ジュール' in text or '熱量' in text:
            return (2, '観察方法_熱量の測定')
        
        # 応用
        if '電熱線' in text or 'ニクロム線' in text:
            return (3, '応用_電熱線')
        if '電熱器' in text or 'ヒーター' in text or '電気製品' in text:
            return (3, '応用_電熱器')
        if '電流' in text and ('大きい' in text or '小さい' in text) and '熱' in text:
            return (3, '応用_電流と発熱の関係')
...
def check_diagrams(questions):
    ...
    try:
        with open('wakaru/physics_current_effect_heating.js', 'r', encoding='utf-8') as f:
            content = f.read()
    except Exception as e:
...
def main():
    ...
    target_file = base_dir / 'wakaru' / 'physics_current_effect_heating.js'
