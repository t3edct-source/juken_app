...
def check_learning_order(questions):
    ...
    def categorize_question(text):
        ...
        # 基礎概念
        if 'てこ' in text and ('何という' in text or '何と' in text or 'つり合い' in text):
            return (0, '基礎概念_てことつり合い')
        if '支点' in text and ('何という' in text or '何と' in text):
            return (0, '基礎概念_支点・力点・作用点')
        
        # 概念（つり合いの仕組み）
        if 'つり合い' in text and ('なぜ' in text or 'どうして' in text or '仕組み' in text):
            return (1, '概念_つり合いの仕組み')
        if '重さ' in text and '距離' in text and ('なぜ' in text or 'どうして' in text):
            return (1, '概念_重さと距離の関係')
        
        # 観察・実験
        if '実験' in text or '観察' in text or '調べる' in text:
            return (2, '観察方法_実験・観察')
        if 'はたらき' in text or '重さ×距離' in text:
            return (2, '観察方法_はたらきの測定')
        
        # 応用
        if '支点' in text and ('端' in text or '真ん中' in text):
            return (3, '応用_支点の位置')
        if '計算' in text or '何g' in text or '何cm' in text:
            return (3, '応用_計算問題')
...
def check_diagrams(questions):
    ...
    try:
        with open('wakaru/physics_lever_weight_basic.js', 'r', encoding='utf-8') as f:
            content = f.read()
    except Exception as e:
    ...
    for q in questions:
        ...
        if source_match:
            ...
            # 図解の有無を確認（<div style= または <img のどちらか）
            has_diagram = '<div style=' in source or '<img' in source
            ...
            if has_diagram:
                ...
                img_match = re.search(r'<img', source)
                ...
                if div_match:
                    ...
                    diagrams_info.append({
                        'qnum': qnum,
                        'has_diagram': True,
                        'title': title if title_match else "画像",
                        'lines': 0,
                        'has_content': True,
                        'type': 'img'
                    })
    ...
    for info in diagrams_info:
        if info['has_diagram']:
            if info['has_content']:
                ...
                diagram_type = info.get('type', 'unknown')
                if diagram_type == 'img':
                    print(f"  Q{info['qnum']}: 図解あり（画像） - 【{info['title']}】")
                else:
    ...
    print()
    print(f"図解あり（内容3行以上または画像）: {diagrams_with_content}問")
...
def main():
    ...
    target_file = base_dir / 'wakaru' / 'physics_lever_weight_basic.js'
