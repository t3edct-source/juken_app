...
# -*- coding: utf-8 -*-
"""
太陽と影（基礎）の単元の問題を推奨順序に再配置（最終版・正確な方法）
"""
...
def extract_all_questions(content):
    ...
    qnum_positions = []
    for match in re.finditer(qnum_pattern, array_content):
        qnum_positions.append((match.start(), int(match.group(1))))
    
    if not qnum_positions:
        return questions
    
    # 各問題オブジェクトを抽出
    for i, (pos, qnum) in enumerate(qnum_positions):
        # この問題オブジェクトの開始位置（直前の { を探す）
        # 行頭から探す
        start_pos = pos
        while start_pos > 0 and array_content[start_pos] != '{':
            start_pos -= 1
        if start_pos > 0:
            # 行の開始位置を探す
            line_start = start_pos
            while line_start > 0 and array_content[line_start-1] != '\n':
                line_start -= 1
            # 実際のオブジェクト開始位置を探す
            for j in range(line_start, start_pos):
                if array_content[j] == '{':
                    start_pos = j
                    break
        
        # 次の問題の開始位置を探す
        if i + 1 < len(qnum_positions):
            next_pos = qnum_positions[i + 1][0]
            # 次の問題の前の { を探す
            end_pos = next_pos
            while end_pos > 0 and array_content[end_pos] != '{':
                end_pos -= 1
            if end_pos > 0:
                # 行の開始位置を探す
                line_start = end_pos
                while line_start > 0 and array_content[line_start-1] != '\n':
                    line_start -= 1
                # 実際のオブジェクト開始位置を探す
                for j in range(line_start, end_pos):
                    if array_content[j] == '{':
                        end_pos = j
                        break
        else:
        ...
        # 問題オブジェクトを抽出
        question_block = array_content[start_pos:end_pos].rstrip()
        # 末尾のカンマを除去
...
def reorder_questions(filepath, new_order):
    ...
    if len(questions) != len(new_order):
        ...
        print(f"見つかった問題: {sorted(questions.keys())}")
...
def main():
    ...
    print("=" * 80)
    print("太陽と影（基礎）の単元 - 問題の再配置（最終版・正確な方法）")
    print("=" * 80)
    print()
